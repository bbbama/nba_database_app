...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare('DELETE FROM trener WHERE id_trenera = ?');
        $stmt->execute([$id_trenera]);
        header('Location: index.php');
        exit;
    } catch (PDOException $e) {
        die("Błąd podczas usuwania trenera: " . $e->getMessage());
    }
}
...
try {
    $stmt = $pdo->prepare("SELECT imie, nazwisko FROM trener WHERE id_trenera = ?");
    $stmt->execute([$id_trenera]);
...
<main>
    ...
    <p>Czy na pewno chcesz usunąć trenera: <strong><?= $itemName ?></strong>?</p>
    
    <form action="delete.php" method="POST" style="display: inline-block;">
        <input type="hidden" name="id" value="<?= htmlspecialchars($id_trenera) ?>">
